...
open( PHENO, "pheno_MATGROUP.txt" )
  || die("Can't the file£º$_!");
open( PEDFILE, ">grinMATGROUP.ped" )
  || die("Can't the file£º$_!");
open( MAPFILE, ">grinMATGROUP.map" )
  || die("Can't the file£º$_!");
...
for ( $i = 0 ; $i < scalar(@PIindexarray) ; $i++ ) {
	...
	if ( $pihm{ @PIindexarray[$i] } eq "000" ) {
		$phetoken = 1;
	}
	elsif ( $pihm{ @PIindexarray[$i] } eq "00" ) {
		$phetoken = 2;
	}
	elsif ( $pihm{ @PIindexarray[$i] } eq "0" ) {
		$phetoken = 3;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "I" ) {
		$phetoken = 4;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "II" ) {
		$phetoken = 5;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "III" ) {
		$phetoken = 6;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "IV" ) {
		$phetoken = 7;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "V" ) {
		$phetoken = 8;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "VI" ) {
		$phetoken = 9;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "VII" ) {
		$phetoken = 10;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "VIII" ) {
		$phetoken = 11;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "IX" ) {
		$phetoken = 12;
	}
elsif ( $pihm{ @PIindexarray[$i] } eq "X" ) {
		$phetoken = 13;
	}
	else {
		print $i. "\t"
