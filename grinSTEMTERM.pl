...
open( PHENO, "pheno_STEMTERM.txt" )
  || die("Can't the file£º$_!");
open( PEDFILE, ">grinSTEMTERM.ped" )
  || die("Can't the file£º$_!");
open( MAPFILE, ">grinSTEMTERM.map" )
  || die("Can't the file£º$_!");
...
for ( $i = 0 ; $i < scalar(@PIindexarray) ; $i++ ) {
	...
	if ( $pihm{ @PIindexarray[$i] } eq "D" ) {
		$phetoken = 1;
	}
	elsif ( $pihm{ @PIindexarray[$i] } eq "N" ) {
		$phetoken = 2;
	}
	elsif ( $pihm{ @PIindexarray[$i] } eq "S" ) {
		$phetoken = 3;
	}
	else {
		print $i. "\t"
